...
export const questions: { id: string; axis: keyof MBTIScores; prompt: string; reverse?: boolean }[] = [
...
export function scoreMBTI(answers: Record<string, number>): { scores: MBTIScores; type: MBTIType } {
  const init: MBTIScores = { IE: 0, SN: 0, TF: 0, JP: 0 };
  for (const q of questions) {
    const raw = answers[q.id] ?? 3;
    const val = (q.reverse ? (6 - raw) : raw) - 3; // -2..+2
    init[q.axis] += val;
  }
